...
contract SimpleNFT {
    ...
    string _baseURL = "https://example.com/images/";
    ...
    function transferFrom(address _from, address _to, uint256 _tokenId) external payable {
        ...
        require(_owners[_tokenId] == _from, "from is not owner of the token");
        require(_from == msg.sender || _operators[_from][msg.sender], "only owner or authorized operators can transfer token");
    ...
    function tokenURI(uint256 _tokenId) external view returns (string memory) {
        require(_owners[_tokenId] != address(0), "no such token");
        return string(abi.encodePacked(_baseURL,_tokenId.toString(),".jpeg"));
    }
